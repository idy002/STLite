...
private:
	...
	void print( Node *nd ) {
		fprintf( stderr, "(" );
		if( nd != pend ) {
			fprintf( stderr, "%d[fhs %d %d %d]", nd->pvalue->first, 
					nd->par->pvalue ? nd->par->pvalue->first : -1, 
					nd->prev->pvalue ? nd->prev->pvalue->first : -1, 
					nd->next->pvalue ? nd->next->pvalue->first : -1 );
			print( nd->lson );
			print( nd->rson );
		}
		fprintf( stderr, ")" );
...
public:
	...
	map(const map &other) {
		if( other.empty() ) {
			pend = new Node;
			pend->prev = pend->next = pend->lson = pend->rson = pend;
